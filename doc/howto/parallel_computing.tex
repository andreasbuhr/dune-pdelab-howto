...
\item High-performance computer have up to 705'024 cores (K computer, Japan).
\end{itemize}
...
\item Developed 1993-94 by an international board
...
OpenMPI can even distinguish automatically between processes which share the same address space and
...
\begin{frame}
  ...
  \begin{itemize}
  \item partition a problem by splitting the domain into small subdomains
...
\item Each entity on a processor has a partition type, which can be determined 
by the method\\
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item Dune parallel programs use a tool to help in setting up and handling the 
...
numberstyle=\tiny, numbersep=2pt,lastline=12]{../../src/course-examples/example01_main.hh}
...
\begin{frame}[fragile]
  \frametitle<presentation>{MPIHelper}
  \begin{itemize}
  \item MPIHelper provides the methods 
 \begin{center}
    \scriptsize
    \begin{tabular}{l|l}
      \hline
    ...
    \end{tabular}    
 \end{center}\medskip
  \item MPIHelper provides the static methods 
 \begin{center}
    \scriptsize
    \begin{tabular}{l|l}
      \hline
      ...
      \lstinline!getCollectiveCommunication! & get communicator\\
...
\end{itemize}
\item There also exists also an algebraic multigrid preconditioner.
\end{itemize}
\end{frame}
...
int overlap=2; 
Dune::YaspGrid<2> grid(mpiHelper.getCommunicator(),L,N,periodic,overlap);
